 ...
 */
#define NUM_COMMAND_OPTS \
	((MAX_COMMANDS > MAX_OPTIONS) ? MAX_COMMANDS : MAX_OPTIONS)
static struct option command_opts[NUM_COMMAND_OPTS];

/**
 * Global optstring used by all subcommands
 */
#define NUM_COMMAND_OPT_STRS (NUM_COMMAND_OPTS * 3)
static char command_optstring[NUM_COMMAND_OPT_STRS];
