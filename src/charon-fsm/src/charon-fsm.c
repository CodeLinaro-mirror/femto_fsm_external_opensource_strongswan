 ...
 * lookup UID and GID
 */
static bool lookup_uid_gid(void)
{
	char *name;
...
static void segv_handler(int signal)
{
	...
	backtrace->log(backtrace, NULL, FALSE);
	backtrace->log(backtrace, stderr, FALSE);
...
static void usage(const char *msg)
{
	...
	fprintf(stderr, "Usage: charon-fsm\n"
		"         [--help]\n"
		"         [--version]\n"
		"         [--use-syslog]\n"
		"         [--debug-<type> <level>]\n"
		"           <type>:  log context type (dmn|mgr|ike|chd|job|cfg|knl|net|"
		"asn|enc|tnc|imc|imv|pts|tls|esp|lib)\n"
		"           <level>: log verbosity (-1 = silent, 0 = audit, 1 = "
		"control,\n"
		"                                    2 = controlmore, 3 = raw, 4 = "
		"private)\n"
		"\n"
		);
}
...
int main(int argc, char *argv[])
{
	...
	if (lib->integrity &&
		!lib->integrity->check_file(lib->integrity, dmn_name, argv[0]))
	{
		library_deinit();
	...
	for (;;)
	{
		struct option long_opts[] = {
			...
			{ "debug-app", required_argument, &group, DBG_APP },
	...
	if (check_pidfile())
	{
		DBG1(DBG_DMN, "%s already running (\""PID_FILE"\" exists)", dmn_name);
		goto deinit;
	}
